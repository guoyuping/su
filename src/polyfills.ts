...
let polyfills = function (win, doc) {
    ...
    var methods = {
        ...
        normalizeHasCaretError: function () {
            if ("createRange" in doc && "getSelection" in win) {
                ...
                if (document.activeElement) {//HTML5增加了辅助管理DOM焦点的功能。首先就是document.activeElement属性，这个属性始终会引用DOM中当前获得焦点的元素。元素获得焦点的方式有页面加载、用户输入（通常是通过按Tab键）和在代码中调用focus()方法
