...
module su{
    ...
    function getRangeNode(node: Node, offset: number) {
    ...
    export class Selection{
        ...
        unselectableClass;
        element;
        constructor() {/*editor, contain, unselectableClass*/
        ...
        //  */
        // setAfter(node, notVisual?, callback?) {
        //     var win = this.win,
        ...
        //                     var caretPlaceholder = this.doc.createTextNode(sulag.INVISIBLE_SPACE);
        //                     node.parentNode.insertBefore(caretPlaceholder, node.nextSibling);
         ...
         */
        selectNode(node, avoidInvisibleSpace?) {
            var range = rangy.createRange(this.doc),
                isElement = node.nodeType === su.ELEMENT_NODE,
                canHaveHTML = "canHaveHTML" in node ? node.canHaveHTML : (node.nodeName !== "IMG"),
                content = isElement ? node.innerHTML : node.data,
                isEmpty = (content === "" || content === su.INVISIBLE_SPACE),
                displayStyle = su.dom.getStyle("display").from(node),
                isBlockElement = (displayStyle === "block" || displayStyle === "list-item");

            if (isEmpty && isElement && canHaveHTML && !avoidInvisibleSpace) {
                // Make sure that caret is visible in node by inserting a zero width no breaking space
                try { node.innerHTML = su.INVISIBLE_SPACE; } catch (e) { }
            }
            if (canHaveHTML) {
        ...
        //     this.setSelection(range);
        // }
        // getSelectedOwnNodes(controlRange?) {
        //     var selection,
        ...
        // getPreviousNode(node, ignoreEmpty) {
        //     var displayStyle;
        //     if (!node) {
        //         var selection = this.getSelection();
        ...
        //         displayStyle = dom.getStyle("display").from(ret);
        //         if (
        //             !lang.array(["BR", "HR", "IMG"]).contains(ret.nodeName) &&
        //             !lang.array(["block", "inline-block", "flex", "list-item", "table"]).contains(displayStyle) &&
        ...
        // getSelectionParentsByTag(tagName) {
        //     var nodes = this.getSelectedOwnNodes(),
        //         curEl, parents = [];

        //     for (var i = 0, maxi = nodes.length; i < maxi; i++) {
        //         curEl = (nodes[i].nodeName && nodes[i].nodeName === 'LI') ? nodes[i] : dom.getParentElement(nodes[i], { query: 'li' }, false, this.contain);
        //         if (curEl) {
        //             parents.push(curEl);
        //         }
        //     }
        ...
        // caretIsInTheEndOfNode(ignoreIfSpaceIsBeforeCaret) {
        ...
        // caretIsFirstInSelection(includeLineBreaks) {
        //     var r = rangy.createRange(this.doc),
        //         s = this.getSelection(),
        //         range = this.getRange(),
        ...
        //     if (startNode) {
        //         if (startNode.nodeType === sulag.TEXT_NODE) {
        //             if (!startNode.parentNode) {
        //                 return false;
        //             }
        //             if (!this.isCollapsed() || (startNode.parentNode.firstChild !== startNode && !dom.domNode(startNode.previousSibling).is.block())) {
        //                 return false;
        //             }
        ...
        //         } else if (includeLineBreaks && dom.domNode(startNode).is.lineBreak()) {
        //             return true;
        //         } else {
        ...
        // getBeforeSelection(includePrevLeaves) {
        ...
        //     inTmpCaret = dom.getParentElement(startNode, { query: '._wysihtml-temp-caret-fix' }, 1);
        //     if (inTmpCaret) {
        ...
        //                 if (prevNode) {
        //                     if (prevNode.nodeType === 1 && dom.hasClass(prevNode, this.unselectableClass)) {
        //                         prevLeaf = prevNode;
        //                     } else {
        //                         prevLeaf = dom.domNode(prevNode).lastLeafNode();
        //                     }
        //                 }
        ...
        includeRangyRangeHelpers() {
            var s = this.getSelection(),
                r = <RangyRange>s.getRangeAt(0),
        ...
        _selectLineUniversal() {
            var s = this.getSelection(),
                ...
                isLineBreakingElement = function (el) {
                    return el && el.nodeType === 1 && (that.win.getComputedStyle(el).display === "block" || lang.array(['BR', 'HR']).contains(el.nodeName));
                },
         ...
         * // [text, text]
         */
        getNodes(nodeType: number | number[], filter?: (node: Node) => boolean) {
            var range = this.getRange();
            if (range) {
                return (range as RangyRange).getNodes(Array.isArray(nodeType) ? nodeType : [nodeType], filter);
            } else {
                return [];
            }
        }
        ...
        _detectInlineRangeProblems(range) {
            var position: any = dom.compareDocumentPosition(range.startContainer, range.endContainer);
            return (
                range.endOffset == 0 &&
         ...
         * 返回选中内容的Range对象
         */
        getRange(dontFix?: boolean): Range {
            var selection = this.getSelection(),
        ...
        isInThisEditable() {
            ...
            return anode && fnode &&
                (dom.contains(this.element, fnode) || this.element === fnode) &&
                (dom.contains(this.element, anode) || this.element === anode);
        }
