...
module su.views {
    ...
    export let actions = {
        ...
        fixLastBrDeletionInTable: function (composer, force?) {
            if (composer.selection.caretIsInTheEndOfNode()) {
                ...
                if (aNode && aNode.nodeType === 1 && (dom.getParentElement(aNode, { query: 'td, th' }, false, composer.element) || force)) {
        ...
        fixDeleteInTheBeginningOfBlock: function (composer) {
            ...
            if (selection.caretIsFirstInSelection(browser.usesControlRanges()) && prevNode) {
                if (prevNode.nodeType === 1 &&
                    dom.domNode(prevNode).is.block() &&
                    ...
                    if ((/^\s*$/).test(prevNode.textContent || prevNode.innerText)) {
                        ...
                        if (prevNode.lastChild) {
                            var selNode = prevNode.lastChild,
                                ...
                                curNode = dom.getParentElement(selectedNode, {
                            ...
                            if (curNode) {
                                ...
                                return true;
                            } else if (browser.usesControlRanges()) {
        ...
        fixDeleteInTheBeginningOfLi: function (composer) {
            if (browser.hasLiDeletingProblem()) {
        ...
        doLineBreaksModeEnterWithCaret: function (composer) {
            ...
            if (caretInfo) {
                ...
                if (parent && caretInfo.caretNode) {
                    ...
                    } else if (caretInfo.caretNode.nodeType === 3 && browser.hasCaretBlockElementIssue() && caretInfo.textOffset === caretInfo.caretNode.data.length && !caretInfo.nextNode) {
    ...
    var handleClick = function (event) {
        if (this.config.classNames.uneditableContainer) {
            ...
            var uneditable = dom.getParentElement(event.target, { query: "." + this.config.classNames.uneditableContainer }, false, this.element);
            if (uneditable) {
    ...
    (su.views.Composer.prototype as any).observe = function () {
        var that = this,
        ...
        actions.addListeners(this.element, ['drop', 'paste', 'beforepaste'], handlePaste.bind(this));
