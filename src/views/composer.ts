...
module su.views {
     ...
     * 设计器
     */
    export class Composer extends views.View {
        name: "textarea" | "composer" = "composer";
        config;
        textarea;
        parent;
        editableArea;
        ...
        constructor(parent, editableElement, config) {
            super(parent, editableElement, config);
            /**
             * Editor中设置
             * 
             */
            if (!this.config.noTextarea) {//editableElement是textarea时
                this.textarea = this.parent.textarea;
            } else {
                this.editableArea = editableElement;
            }
            if (this.config.contentEditableMode) {//editableElement不是textarea时
         ...
         */
        getValue(parse: boolean, clearInternals: boolean): string {
            let value = this.isEmpty() ? "" : quirks.getCorrectInnerHTML(this.element);
            if (parse !== false) {
         ...
         */
        setValue(html: string, parse: boolean): void {
            if (parse !== false) {
        ...
        _createWysiwygFormField() {
            if (this.textarea.element.form) {
                var hiddenField: any = document.createElement("input");
        ...
        _create() {
            ...
            if (name) {
                ...
                if (!this.config.contentEditableMode) { su.dom.addClass(this.editableArea, name); }
            }
