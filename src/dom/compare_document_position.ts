...
module su.dom {
    export function compareDocumentPosition(container, element) {
        ...
        if (documentElement.compareDocumentPosition) {
            ...
            return function (container, element) {
                ...
                while (point) {
                    ...
                    var location_index = lang.array(parents).indexOf(point);
                    if (location_index !== -1) {
                        var smallest_common_ancestor = parents[location_index];
                        var this_index = lang.array(smallest_common_ancestor.childNodes).indexOf(parents[location_index - 1]);//smallest_common_ancestor.childNodes.toArray().indexOf( parents[location_index - 1] );
                        var other_index = lang.array(smallest_common_ancestor.childNodes).indexOf(previous); //smallest_common_ancestor.childNodes.toArray().indexOf( previous );
