...
module su.dom {
    ...
    export let  parse = function(elementOrHtml_current, config_current) {
            ...
            if (!newNode) {
                if (newNode === false) {
                    ...
                    if (nodeDisplay === '') {
                        ...
                        nodeDisplay = lang.array(blockElements).contains(oldNode.tagName) ? "block" : "";
                    }
                    if (lang.array(["block", "flex", "table"]).contains(nodeDisplay)) {
        ...
        function _applySelectorRules(element, selectorRules) {
            ...
            for (sel in selectorRules) {
                if (selectorRules.hasOwnProperty(sel)) {
                    if (lang.object(selectorRules[sel]).isFunction()) {
                    ...
                    if (definition.styles.hasOwnProperty(s)) {
                        for (var sp = nodeStyles.length; sp--;) {
                            ...
                            if (styleProp[0].replace(/\s/g, '').toLowerCase() === s) {
                                if (definition.styles[s] === true || definition.styles[s] === 1 || lang.array(definition.styles[s]).contains(styleProp[1].replace(/\s/g, '').toLowerCase())) {
                                    return true;
                                }
                            }
                        }
        ...
        function _checkAttribute(attributeName, attributeValue, methodName, nodeName): string | boolean {
            var method = su.lang.object(methodName).isFunction() ? methodName : attributeCheckMethods[methodName],
                newAttributeValue;
        ...
        function _checkAttributes(oldNode, local_attributes) {
            var globalAttributes = su.lang.object(currentRules.attributes || {}).clone(), // global values for check/convert values of attributes
                checkAttributes = su.lang.object(globalAttributes).merge(su.lang.object(local_attributes || {}).clone()).get(),
                attributes = {},
                oldAttributes = su.dom.getAttributes(oldNode),
        ...
        function _handleAttributes(oldNode, newNode, rule, clearInternals) {
            let attributes: any = {},                         // fresh new set of attributes to set on newNode
            ...
            if (addStyle) {
                for (attributeName in addStyle) {
                    ...
                    let newStyle = method(su.dom.getAttribute(oldNode, attributeName));
    ...
    export let getAsDom = (function () {
        ...
        return function (html, context) {
            ...
            } else if (browser.supportsHTML5Tags(context)) {
                tempElement = context.createElement("div");
