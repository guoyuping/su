    ...
    export function domNode(node) {
        ...
        return {
            ...
            prev: function (options) {
                ...
                if (
                    dom.domNode(prevNode).is.rangyBookmark() || // is Rangy temporary boomark element (bypass)
                    (!su.lang.array(types).contains(prevNode.nodeType)) || // nodeTypes check.
            ...
            next: function (options) {
                ...
                if (
                    dom.domNode(nextNode).is.rangyBookmark() || // is Rangy temporary boomark element (bypass)
                    (!su.lang.array(types).contains(nextNode.nodeType)) || // nodeTypes check.
            ...
            test: function (properties) {
                ...
                if (properties.styleProperty && properties.styleProperty.length > 0) {
                    ...
                    for (var j = 0, maxStyleP = styles.length; j < maxStyleP; j++) {
                        ...
                        prop = browser.fixStyleKey(styles[j]);
                        if (node.style[prop]) {
                ...
                if (properties.attribute) {
                    var attr = dom.getAttributes(node),
                        attrList = [],
