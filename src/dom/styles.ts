...
module su.dom {
  ...
  var shouldIgnoreBoxSizingBorderBox = function (element) {
    if (hasBoxSizingBorderBox(element)) {
      return parseInt(getStyle("width").from(element), 10) < element.offsetWidth;
    }
    return false;
  };
  ...
  var hasBoxSizingBorderBox = function (element) {
    ...
    for (; i < length; i++) {
      if (getStyle(BOX_SIZING_PROPERTIES[i]).from(element) === "border-box") {
   ...
   *
   */
  export function copyStyles(stylesToCopy) {
    return {
